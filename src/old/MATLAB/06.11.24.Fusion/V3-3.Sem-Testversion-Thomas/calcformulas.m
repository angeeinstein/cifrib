  function [q,F,Mb] = calcformulas(l)
   ...
   %Fn Bearingforce
    ...
    boundary_conditions = [subs(F, x, l*1.01) == 0, subs(Mb, x, l*1.01) == 0];
     ...
     [~, j5] = size(main.Joint);
                for i = 1:j5
                    ...
                    % if main.Joint(2,i) == 0
                    % boundary_conditionsFN = [boundary_conditions,subs(Fn, x, main.Bearing(1,i)) == 0];
                    % end
    ...
    [~, j6] = size(main.Bearing);

    % Nur die Feldnamen herausfiltern, die mit 'F' beginnen
feldNamen = fieldnames(sol);
F_felder = feldNamen(startsWith(feldNamen, 'F'));

% Nur die F-Felder in ein neues Struct übernehmen
solF = rmfield(sol, setdiff(feldNamen, F_felder));

% Extrahiere und wandle die Werte der gefilterten Felder in numerische Werte um
matrixWerte = cell2mat(cellfun(@double, struct2cell(solF), 'UniformOutput', false));

% Ausgabe der Matrix mit numerischen Werten
...
disp(matrixWerte);
    ...
    myApp.UITable.Data = bearingractions();
